...
const kakaoMsgFormatter = format.combine(
  format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }),
  format.printf((info) => {
    return `${info.timestamp} [${info.level.toUpperCase()}]: ${info.message} .`
  })
...
const adminLogFormatter = format.combine(
  format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }),
  format.printf((info) => {
    // console.log("info:",info);
    return `[${info.timestamp}] [${info.level.toUpperCase()}] [${
      info?.message?.user?.userName
    }] [${info.message?.ip}] ${info.message?.path}\n-Body:${JSON.stringify(
      info?.message?.body
    )}\n-Resp:${JSON.stringify(info.message?.resp)} .`
  })
)

const adminLogExportFormatter = format.combine(
  format.timestamp({ format: "YYYY-MM-DD HH:mm:ss" }),
  format.printf((info) => {
    // console.log("info:",info);
    return `[${info.timestamp}] [${info?.message?.user?.userName}] [${
      info.message?.ip
