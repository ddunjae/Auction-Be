  ...
  return knex.schema.hasTable("crawling").then(function (exists) {
    if (!exists) {
      return knex.schema.createTable("crawling", function (property) {
        ...
        property.decimal("height", 10, 3); // Cao
        property.decimal("width", 10, 3); // rộng
        property.decimal("depth", 10, 3); // sâu
        ...
        property.integer("size_table", 20); // Số hiệu
        // More info
        ...
        property.text("codition_report"); // báo cáo điều kiện
        property.text("description"); // mô tả
        // Number
        property.string("currency", 255).notNullable(); // Loại tiền
        property.float("start_price", 15, 6);
        property.float("hammer_price", 15, 6);
        property.float("selling_price", 15, 6);
        property.float("estimate_min", 15, 6);
        property.float("estimate_max", 15, 6);
        property.float("usd_start_price", 15, 6);
        property.float("usd_hammer_price", 15, 6);
        property.float("usd_selling_price", 15, 6);
        property.float("usd_estimate_min", 15, 6);
        property.float("usd_estimate_max", 15, 6);
        property.float("competiton", 15, 6); // Tỉ lệ cạnh tranh
