  ...
  return knex.schema.hasTable("user").then(function (exists) {
    if (!exists) {
      return knex.schema.createTable("user", function (property) {
        ...
        property.boolean("active").defaultTo(false); // tài khoản đã được active hay chưa
        ...
        property.string("verified_phone_number", 255);
        property.string("pw_encrypt_method", 20).defaultTo("NEW");
        property
          .string("id_origin_artnguide", 255)
          .comment("Id user từ bảng của artnguide cũ");
        property
          .string("clone_of_uid", 255)
          .comment("clone của user id nào, trường hợp đối với naver, kakao");
